...
void gf3d_extensions_device_init(VkPhysicalDevice device, const char *config)
{
    ...
    if (!gf3d_device_extensions.available_extension_count)
        return;
...
void gf3d_extensions_instance_init(const char *config)
{
    ...
    if (!gf3d_instance_extensions.available_extension_count)
        return;
...
{
    if (count != NULL)
        *count = gf3d_instance_extensions.enabled_extension_count;
...
{
    if (count != NULL)
        *count = gf3d_device_extensions.enabled_extension_count;
