...
void gf2d_draw_window_border_tiled(Sprite *border, Sprite *bg, Rect rect, Color color)
{
    ...
    if ((border->frameWidth == 0) || (border->frameHeight == 0))
        return;
...
void gf2d_draw_window_border_stretched(Sprite *border, Sprite *bg, Rect rect, Color color)
{
    ...
    if ((!border) || (border->frameHeight == 0) || (border->frameWidth == 0))
        return;
            ...
            if (window)
            {
                ...
                if (str)
                {
                    if (strcmp(str, "tiled") == 0)
                        window_manager.style = WDS_Tiled;
                    else if (strcmp(str, "stretched") == 0)
                        window_manager.style = WDS_Stretched;
                }
            }
...
Window *gf2d_window_load(char *filename)
{
    ...
    {
        if (!strlen(win->name))
            gfc_line_cpy(win->name, filename);
